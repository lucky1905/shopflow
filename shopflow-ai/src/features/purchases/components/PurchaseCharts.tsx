...
import type { PoStatusPoint, SpendTrendPoint, SupplierSpendPoint } from '../types';
        ...
        {payload.map((entry) => (
          <p key={String(entry.dataKey)} className="flex items-center gap-2">
            ...
            <span className="ml-auto font-bold tabular-nums text-foreground">
              {typeof entry.value === 'number' && String(entry.dataKey) !== 'orders'
...
export function SpendTrendChart({
  ...
  return (
    <SectionCard
      title="Spend trend"
      description="Daily purchase spend across the selected range"
      action={action}
    >
      <ChartBody
        isLoading={isLoading}
      ...
      >
        <AreaChart data={data} margin={{ top: 8, right: 8, left: 0, bottom: 0 }}>
          <defs>
            <linearGradient id="purchase-spend" x1="0" y1="0" x2="0" y2="1">
...
export function SupplierSpendChart({
  ...
  return (
    <SectionCard title="Spend by supplier" description="Top suppliers over the last 90 days">
      <ChartBody
        isLoading={isLoading}
        error={error}
        onRetry={onRetry}
...
export function PurchaseStatusChart({
  ...
  return (
    <SectionCard title="Order status mix" description="All purchase orders by lifecycle state">
      <ChartBody
        isLoading={isLoading}
        error={error}
        onRetry={onRetry}
      ...
      <ul className="mt-4 grid grid-cols-2 gap-2 border-t border-border pt-3.5">
        {data.map((point) => (
          <li key={point.name} className="flex items-center gap-2 text-sm">
            <span
